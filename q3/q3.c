...
void *reader(int *n)
{
  ...
  while (1)
  {
    ...
    while (w_status == W_SOLICITANDO) // Trava todas as threads de leitura e espera a escrita ser realizada
    ...
    pthread_mutex_lock(&mtx_rcount);
    printf("Leitora %d realizou uma leitura e retornou o valor %c! Restam: %d", *n, valor, r_count);
    if (--r_count == 0)
...
void *escrever(int *n)
{
  while (1)
  {
    ...
    printf("Escritora %d está aguardando para escrever!", *n);
    ...
    printf("Escritora %d realizou uma escrita!", *n);
